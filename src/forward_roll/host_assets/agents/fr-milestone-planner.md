...
2. Update `PROJECT.md` only when the milestone intent or a key decision must change.
3. Update `REQUIREMENTS.md` only when milestone-scoped requirements or traceability must change.
4. Update `ROADMAP.md` with the next milestone phases and tasks while preserving durable global numbering.
5. Update `STATE.md` so current focus, progress, and next-step guidance remain consistent with the roadmap.
...
- the update would need files outside `PROJECT.md`, `REQUIREMENTS.md`, `ROADMAP.md`, and `STATE.md`
